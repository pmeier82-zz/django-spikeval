...
class Module(ModDefault):
    """module__ for default visuals - for use in the django context"""
    ...
    def save(self, mod, ev):
        ...
        # check for results
        if self._stage != 3:
            raise ModuleExecutionError("save initiated when module__ was not finalised!")

        # per result saving:
        for i, res in enumerate(self.result):
            # result
            kind = (i + 1) * 10
            rval = ResultDefaultVisual(analysis=ev, module=mod, kind=kind)
            ...
            # file
            filename = "eval{}_{}.png".format(ev.id, KINDS.get(kind, "unknown{}".format(i)))
